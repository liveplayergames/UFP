    ...
    /**
     * Created by kaandoit on 12/14/16.
     */
    ...
    public class WelcomeActivity extends AppCompatActivity implements HTTP_Query_Client, Payment_Processor_Client {
        ...
        private long ufp_backend_block_no = -2;
	    ...
	    private static final int DUPLICATE_ACCOUNT = 9;
                ...
                public void handleMessage(Message message) {
                    switch (message.what) {
	        	        ...
	        	        case DUPLICATE_ACCOUNT: {
                            show_ok_dialog(R.string.duplicate_account_title, R.string.duplicate_account_msg);
    			            break;
			            }
                        //
                        ...
                        case SHOW_ETH_SITES_DIALOG: {
                            if (!we_are_dead) {
                                ...
                                long ufp_backend_block_no = scanandchain.second.longValue();
                                if (toast != null)
                                    toast.cancel();
                                String toast_msg_pre = getResources().getString(R.string.partner_sites_down);
                                (toast = Toast.makeText(context, toast_msg_pre +
                                        "\n" + etherscan_io_block_no + " != " + ufp_backend_block_no, Toast.LENGTH_LONG)).show();
                                ...
                                String error = "etherscan.io block number, " + etherscan_io_block_no + " != unclefinneys.com block number, " + ufp_backend_block_no;
                                msg = msg.replace("ERROR", error);
        ...
        //verifiy that our server and etherscan.io are up, and in agreement about the current block number.
        ...
        private void chk_eth_sites_are_ok() {
            ...
            ufp_backend_block_no = -2;
            String ufp_backend_URL = "http://ufp.liveplayergames.com:3000/current_block";
            String ufp_backend_parms[] = new String[2];
            ufp_backend_parms[0] = ufp_backend_URL;
            ufp_backend_parms[1] = "ufp_backend_block_count";
            System.out.println("chk_eth_sites_are_ok: initiate read from ufp backend");
            new HTTP_Query_Task(this, context).execute(ufp_backend_parms);
        ...
        private void verify_eth_sites_are_ok() {
            System.out.println("verify_eth_sites_are_ok: have " + etherscan_io_block_no + " and " + ufp_backend_block_no);
            if (etherscan_io_block_no == 0 || ufp_backend_block_no == 0) {
                if (toast != null)
                    toast.cancel();
            ...
            if (etherscan_io_block_no < 0 || ufp_backend_block_no < 0) {
                ...
                System.out.println("waiting for the other of etherscan.io or ufp backend...");
                return;
            }
            if (Math.abs(etherscan_io_block_no - ufp_backend_block_no) > 5) {
                System.out.println("verify_eth_sites_are_ok: calling SHOW_ETH_SITES_DIALOG");
                Pair<Long, Long> scanandchain = new Pair<>(new Long(etherscan_io_block_no), new Long(ufp_backend_block_no));
        ...
        public void handle_http_rsp(String callback, String rsp) {
          ...
          if (callback.equals("register")) {
              ...
              if (rsp.isEmpty()) {
                    ...
                    if (status_str.equals("account is already in use")) {
                      Button play_button = (Button) findViewById(R.id.play);
                      play_button.setEnabled(false);
                      Message message = message_handler.obtainMessage(DUPLICATE_ACCOUNT, 0, 0, 0);
                      message.sendToTarget();
                      show_whats_new();
		                ...
		                Button play_button = (Button) findViewById(R.id.play);
                        play_button.setEnabled(false);
		                if (status_str.contains("username")) {
            ...
            //
            if (callback.equals("ufp_backend_block_count")) {
                //typical:
	        // 4316417
                ufp_backend_block_no = 0;
                String count_str = rsp;
                if (!count_str.isEmpty()) {
                    try {
                        ufp_backend_block_no = Long.parseLong(count_str);
                    } catch (Exception e) {
                        System.out.println(e.toString() + " (count_str = " + count_str + ")");
                    }
                }
                //will continue to next step of initialization if we have results from both our server and etherscan.io
