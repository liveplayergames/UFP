...
public class HistoryActivity extends AppCompatActivity implements HTTP_Query_Client {
  ...
  protected void onCreate(Bundle savedInstanceState) {
    ...
    (toast = Toast.makeText(context, getResources().getString(R.string.retrieving_transactions) + "...", Toast.LENGTH_LONG)).show();
    String app_name = getResources().getString(R.string.app_name);
    toolbar.setTitle(app_name);
    toolbar.setSubtitle(show_sent ? getResources().getString(R.string.Payments_Sent) : getResources().getString(R.string.Payments_Received));
  }
  ...
  private int set_transactions(String rsp) {
    ...
    if (status != 1) {
      Util.show_err(getBaseContext(), getResources().getString(R.string.error_retrieving_transactions), 3);
  ...
  public void handle_http_rsp(String callback, String rsp) {
    if (callback.equals("transactions")) {
      ...
      if ((processed) >= 50) {
        ...
        if (toast != null)
          toast.cancel();
        String toast_msg = getResources().getString(R.string.retrieving_X_transactions);
        toast_msg = toast_msg.replace("X", String.valueOf(total_transactions_processed));
