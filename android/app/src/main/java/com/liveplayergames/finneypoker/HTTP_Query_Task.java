...
import static com.liveplayergames.finneypoker.BuildConfig.DEBUG;

/**
 * Created by kaandoit on 11/25/16.
 */
public class HTTP_Query_Task extends AsyncTask<String, Void, Void> {
    ...
    protected void onPostExecute(Void result) {
        System.out.println("HTTP_Query_Task::onPostExecute");
       if (exception_occurred) {
           //toast LENGTH_LONG is 3.5 secs. (and LENGTH_SHORT is 2.5) we need to display this about 5 secs
            String msg = context.getResources().getString(R.string.error_check_connection);
            Util.show_err(context, msg, 5);
    ...
    public void send_http_query(String url_string) {
        ...
        if (url == null) {
            ...
            exception_msg = context.getResources().getString(R.string.dns_exception);
            return;
        }
        if (BuildConfig.DEBUG)
        ...
        try {
            ...
            exception_occurred = true;
            exception_msg = context.getResources().getString(R.string.socket_exception);
            if (BuildConfig.DEBUG)
        ...
        if (BuildConfig.DEBUG) {
            System.out.println("send_http_query: response: " + http_query_rsp);
